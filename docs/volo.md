...
<span class="champIntro">Volo is supposed to be unaffiliated (but lacks the tag for it and so technically isn't) and normally benefits from the following stat nodes:</span>
...
The Magic core is so far ahead of the other cores that it's really the only choice for Volo. Though it is worth noting that it doesn't have any health nodes.
...
Note that the damage of the Dexterous core varies depending on the number of 15+ Dexterity champions in the formation. The damage numbers on the layout above account for just 1 (Volo only). For a specific breakdown of how the core fares with more 15+ Dexterity champions - check the table below.
